...
class Admin extends React.Component{
  ...
  runBigBoard() {
    ...
    .then((doc) => {
      doc.docs.map(function(prospectDoc) {
        ...
        db.collection("class2028").doc(fullName).update({
  ...
  runCombine() {
    ...
    db.collection("class2028").where("Tier", ">", -1)
    .get()
    .then((doc) => {
      doc.docs.map(function(prospectDoc) {
        ...
        db.collection('combine2028').doc(fullNameLowerCase).set({
    ...
    if (currentUID !== adminUID) { //|| currentUID !== adminUID2 || currentUID !== adminUID3) {
      ...
      db.collection("franchises").get().then(function(querySnapshot) {
        querySnapshot.docs.map(function(teamDoc) {
          ...
          let abrev = teamDoc.data().abrev;
          // use url property of each element in array to find doc in class2028 collection
          interviewList.forEach(function(prospect) {
            let docRef = db.collection('class2028').doc(prospect.url);
