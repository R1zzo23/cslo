...
class Admin extends React.Component{
    ...
    if (currentUID !== adminUID) { //|| currentUID !== adminUID2 || currentUID !== adminUID3) {
      ...
      .then((doc) => {
        doc.docs.map(function(prospectDoc) {
          ...
          db.collection("class2030").doc(fullName).update({
  ...
  runCombine() {
    ...
    if (currentUID !== adminUID) { //|| currentUID !== adminUID2 || currentUID !== adminUID3) {
      ...
      db.collection("class2030").where("Tier", ">", -1)
      .get()
      .then((doc) => {
        doc.docs.map(function(prospectDoc) {
          ...
          db.collection('combine2030').doc(fullNameLowerCase).set({
    ...
    if (currentUID !== adminUID) { //|| currentUID !== adminUID2 || currentUID !== adminUID3) {
      ...
      db.collection("franchises").get().then(function(querySnapshot) {
        querySnapshot.docs.map(function(teamDoc) {
          ...
          let abrev = teamDoc.data().abrev;
          // use url property of each element in array to find doc in class2030 collection
          interviewList.forEach(function(prospect) {
            let docRef = db.collection('class2030').doc(prospect.url);
